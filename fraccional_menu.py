"""
fraccional_menu.py — Menú principal de Fraccional
Uso: python3 fraccional_menu.py
...
def menu_actualizar():
    while True:
        ...
        sel = questionary.select(
            "",
            choices=[
                questionary.Choice("  Actualizar PN y PJ", value="ambos"),
                questionary.Choice("  Actualizar solo PN", value="pn"),
                questionary.Choice("  Actualizar solo PJ", value="pj"),
        ...
        try:
            import fraccional as frac
            frac.init_db()
            frac.bw_unlock()
            extracted_at = datetime.datetime.now(datetime.timezone.utc).isoformat()
            run_pj = sel in ("ambos", "pj")
            run_pn = sel in ("ambos", "pn")
